         ...
         all.each do |h|
             ...
             if len == 0
               ...
               if (diff > dea) and (rec['diff']< rec['dea'])
                 ...
                 puts "at #{h[:time]} #{peroid/60}m  diff>dea , #{get_name_str(h[:code])} suggest to buy  at #{price}" if show_log
                ...
                if (diff < dea) and (rec['diff'] > rec['dea'])
                 ...
                 puts "at #{h[:time]} #{peroid/60}m diff<dea , #{get_name_str(h[:code])} suggest to sell at #{price}" if show_log
